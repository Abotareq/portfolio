...
export default function Hero() {
  ...
  const active = useSceneActive(ref, '0px')
    ...
    <section id="home" ref={ref} className="relative min-h-[100svh] overflow-hidden flex items-center noise">
      <div className="absolute inset-0">
        <Suspense fallback={null}>
          <HeroScene scrollRef={scrollRef} theme={theme} isRTL={isRTL} active={active} />
        </Suspense>
      </div>
      ...
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-r rtl:bg-gradient-to-l from-bg/80 via-bg/30 to-transparent" />
      ...
      <motion.div style={{ y: contentY, opacity: contentOpacity }} className="container-x relative z-10 pt-28 pb-20">
        <motion.div variants={container} initial="hidden" animate="show" className="max-w-3xl">
          ...
          <motion.h1 variants={item} className="mt-3 text-4xl sm:text-6xl md:text-7xl font-extrabold tracking-tight text-white leading-[1.05]">
            {first}
            {rest && <span className="block text-slate-500">{rest}</span>}
          </motion.h1>
          <motion.h2 variants={item} className="mt-4 text-2xl sm:text-3xl md:text-4xl font-bold tracking-tight">
            ...
            <span className="inline-block text-slate-500" dir="ltr">
              {t('hero.subtitle')}
            </span>
          </motion.h2>
          ...
          <motion.div variants={item} className="mt-9 flex flex-wrap gap-3">
            <a href="#projects" onClick={go('projects')} className="btn-primary group">
              {t('hero.viewProjects')} <FiArrowRight className="transition-transform group-hover:translate-x-0.5 rtl:-scale-x-100 rtl:group-hover:-translate-x-0.5" />
            </a>
          ...
          <motion.div variants={item} className="mt-12 flex flex-wrap gap-x-6 gap-y-2 font-mono text-xs text-slate-500">
            {['ASP.NET Core', 'React', 'SQL Server', 'Node.js', 'MongoDB', 'TypeScript'].map((tech) => (
      ...
      >
        ...
        <span className="relative h-9 w-5 rounded-full border border-white/20">
          <motion.span animate={{ y: [4, 16, 4], opacity: [1, 0.2, 1] }} transition={{ duration: 1.8, repeat: Infinity, ease: 'easeInOut' }} className="absolute left-1/2 top-0 h-2 w-1 -translate-x-1/2 rounded-full bg-accent" />
        </span>
      </motion.a>
    </section>
  )
