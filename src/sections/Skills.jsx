...
const TechConstellation = lazy(() => import('../three/TechConstellation'))
    ...
    <section id="skills" className="section">
      ...
      <div className="container-x relative">
        <SectionHeader
          ...
          lede="Extracted directly from my resume. Hover the constellation to explore the stack."
        />

        <div className="mt-14 grid gap-6 lg:grid-cols-5">
          <Reveal className="lg:col-span-2">
            <div className="card relative h-[360px] lg:h-full min-h-[420px] overflow-hidden">
              <div className="absolute left-4 top-4 z-10 eyebrow !text-[10px]">Tech constellation</div>
                  ...
                  <TiltCard intensity={5} className="h-full">
                    <div className="card card-hover relative h-full p-5">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="ml-auto font-mono text-xs text-slate-500">{group.items.length}</span>
                      </div>
                      <motion.ul variants={list} initial="hidden" whileInView="show" viewport={{ once: true }} className="mt-4 flex flex-wrap gap-2">
                        {group.items.map((s) => (
                          <motion.li key={s} variants={chip} whileHover={{ y: -2, scale: 1.04 }} className="chip hover:border-accent/40 hover:text-white cursor-default">
                            {s}
                          </motion.li>
                        ))}
                      </motion.ul>
